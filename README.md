...
Set `MYCODEAGENT_WORKTREE_ROOT` to use a shorter worktree location, which can
help Windows installations with restrictive path-length policies. Git pathspecs
are emitted with forward slashes on every platform. The runtime uses
